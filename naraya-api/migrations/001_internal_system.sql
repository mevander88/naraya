...
CREATE INDEX IF NOT EXISTS naraya_library_user_updated_idx ON naraya_library_items (user_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS naraya_library_bookmark_idx ON naraya_library_items (user_id, is_bookmarked, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS naraya_comments_comic_idx ON naraya_comments (comic_slug, created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS naraya_comments_chapter_idx ON naraya_comments (chapter_slug, created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS naraya_comments_parent_idx ON naraya_comments (parent_id, created_at ASC) WHERE deleted_at IS NULL;
