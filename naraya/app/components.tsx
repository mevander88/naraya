  ...
  return (
    <section className="content-visibility-auto mt-14 px-container-mobile md:px-container-desktop">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="font-display text-2xl font-semibold text-on-background">{title}</h2>
        <Link href={href} prefetch={false} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-surface-container-high px-4 py-2 text-sm font-semibold text-primary transition hover:border-primary/50 hover:bg-primary/10">
          Lihat semua
          <ArrowRight size={16} />
        </Link>
      </div>
...
export const ComicCard = memo(function ComicCard({ comic, priority = false }: { comic: ComicCardData; priority?: boolean }) {
  return (
    <article className="reveal-soft group relative w-full min-w-0 shrink-0 snap-start cursor-pointer">
      <Link href={detailHref(comic)} prefetch={false} className="relative mb-3 block aspect-[2/3] overflow-hidden rounded-xl border border-white/5">
      ...
      <Link href={detailHref(comic)} prefetch={false} className="block truncate text-lg font-semibold text-on-surface transition group-hover:text-primary">{comic.title}</Link>
  ...
  return (
    <section className="content-visibility-auto mt-20 px-container-mobile pb-16 md:px-container-desktop">
      <div className="mb-8 flex items-center justify-between">
        <h2 className="font-display text-2xl font-semibold text-on-background">{title}</h2>
        <Link href="/komik" prefetch={false} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-surface-container-high px-4 py-2 text-sm font-semibold text-primary transition hover:border-primary/50 hover:bg-primary/10" aria-label="Buka indeks komik dan anime">
          <Filter size={18} />
          Indeks
        </Link>
      </div>
      <div className="grid grid-cols-2 gap-x-5 gap-y-7 md:grid-cols-4 md:gap-x-6 lg:grid-cols-6">
        {lead ? <Link href={latestHref(lead)} prefetch={false} className="interactive-lift group col-span-2 row-span-2 min-h-[360px] cursor-pointer overflow-hidden rounded-2xl border border-white/5">
        ...
        {secondaryUpdates.map((comic) => (
          <Link key={`${comic.kind || 'comic'}-${comic.slug || comic.title}`} href={latestHref(comic)} prefetch={false} className="interactive-lift group cursor-pointer">
