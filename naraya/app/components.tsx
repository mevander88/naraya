...
import { ComicCardData } from './data';
...
export function ContinueReading({ comic }: { comic?: ComicCardData }) {
  ...
  return (
    <section className="relative z-10 -mt-10 px-container-mobile md:px-container-desktop">
      <div className="glass-panel reveal-soft flex flex-col gap-6 rounded-2xl p-5 md:flex-row md:items-center md:p-6">
...
export function GenreChips({ activeGenre = 'All', genres = [] }: { activeGenre?: string; genres?: string[] }) {
  if (!genres.length) return null;

  return (
    <section className="content-visibility-auto mt-14 px-container-mobile md:px-container-desktop">
      <h3 className="font-display text-2xl font-semibold text-on-background">Genre Populer</h3>
...
export function TrendingRail({ title = 'Trending Hari Ini', href = '/explore', comics = [] }: { title?: string; href?: string; comics?: ComicCardData[] }) {
  return (
    <section className="content-visibility-auto mt-14 px-container-mobile md:px-container-desktop">
      <div className="mb-6 flex items-center justify-between">
        <h3 className="font-display text-2xl font-semibold text-on-background">{title}</h3>
...
export function ComicCard({ comic, priority = false }: { comic: ComicCardData; priority?: boolean }) {
  return (
    <article className="reveal-soft group relative w-full min-w-0 shrink-0 snap-start cursor-pointer">
      <Link href={detailHref(comic)} className="relative mb-3 block aspect-[2/3] overflow-hidden rounded-xl border border-white/5">
          <img src={comic.image} alt={comic.title} width={224} height={336} loading={priority ? 'eager' : 'lazy'} fetchPriority={priority ? 'high' : 'auto'} decoding="async" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      ...
      <Link href={detailHref(comic)} className="block truncate text-lg font-semibold text-on-surface transition group-hover:text-primary">{comic.title}</Link>
      <p className="text-sm text-on-surface-variant">{comic.meta || detailLabel(comic)}</p>
    </article>
  );
}

export function UpdatesGrid({ title = 'Update Terbaru', comics = [] }: { title?: string; comics?: ComicCardData[] }) {
  const liveUpdates = comics.slice(0, 18);
  const lead = liveUpdates[0];
  const secondaryUpdates = liveUpdates.slice(1);

  return (
    <section className="content-visibility-auto mt-20 px-container-mobile pb-16 md:px-container-desktop">
      ...
      <div className="grid grid-cols-2 gap-x-5 gap-y-7 md:grid-cols-4 md:gap-x-6 lg:grid-cols-6">
        {lead ? <Link href={latestHref(lead)} className="interactive-lift group col-span-2 row-span-2 min-h-[360px] cursor-pointer overflow-hidden rounded-2xl border border-white/5">
          <div className="relative h-full">
            <img src={lead.image} alt={lead.title} width={520} height={780} loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
            ...
            <div className="absolute bottom-6 left-6 right-6">
              ...
              <h4 className="mt-3 font-display text-2xl font-semibold text-white">{lead.title}</h4>
        ...
        {secondaryUpdates.map((comic) => (
          <Link key={`${comic.kind || 'comic'}-${comic.slug || comic.title}`} href={latestHref(comic)} className="interactive-lift group cursor-pointer">
            <div className="mb-3 aspect-[2/3] overflow-hidden rounded-xl border border-white/5">
              <img src={comic.image} alt={comic.title} width={180} height={270} loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
            </div>
