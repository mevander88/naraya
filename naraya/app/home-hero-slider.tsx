    ...
    <section className="relative h-[560px] overflow-hidden md:h-[660px]">
      {hero ? (
        <img
          ...
          decoding="async"
          sizes="100vw"
          className="image-render-safe absolute inset-0 h-full w-full animate-[hero-kenburns_620ms_ease-out_both] object-cover"
        />
      ) : null}
      <div className="absolute inset-0 bg-gradient-to-t from-background via-background/55 to-transparent" />
      <div className="absolute inset-0 bg-gradient-to-r from-background/90 via-background/25 to-transparent" />
      <div key={`copy-${heroKey}`} className="relative flex h-full max-w-5xl animate-[hero-copy_440ms_cubic-bezier(0.25,1,0.5,1)_both] flex-col justify-end px-container-mobile pb-24 md:px-container-desktop md:pb-32">
        <span className="mb-3 text-xs font-semibold uppercase tracking-[0.28em] text-primary">Update Naraya</span>
        <h1 className="line-clamp-3 max-w-3xl font-display text-[2.35rem] font-bold leading-[1.02] text-on-background md:line-clamp-none md:text-6xl">{hero?.title ?? 'Naraya Comic Feed'}</h1>
