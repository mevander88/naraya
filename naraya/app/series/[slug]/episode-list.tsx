...
import { ArrowDownUp, ChevronDown, Film, Play } from 'lucide-react';
...
export function EpisodeList({ episodes }: { episodes: SeriesEpisodeData[] }) {
  ...
  const sortedEpisodes = useMemo(() => sortList(episodes, sortOrder), [episodes, sortOrder]);
  const shouldCollapse = sortedEpisodes.length > 5;
  const visibleEpisodes = expanded || !shouldCollapse ? sortedEpisodes : sortedEpisodes.slice(0, 5);
    ...
    <div className="relative min-w-0 overflow-hidden rounded-[2rem] bg-[linear-gradient(145deg,rgba(29,26,35,0.98),rgba(21,18,27,0.96))] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.055),0_24px_64px_rgba(0,0,0,0.32)] md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            {shouldCollapse && !expanded ? `Menampilkan 5 episode ${sortLabel}.` : 'Pilih episode untuk mulai menonton.'}
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-2.5">
        {visibleEpisodes.map((episode) => (
          <Link key={episode.slug} href={`/nonton/${episode.slug}`} className="group flex min-w-0 items-center justify-between gap-4 rounded-2xl bg-background/34 px-3 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.045),0_12px_30px_rgba(0,0,0,0.16)] transition hover:bg-primary/10 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_16px_34px_rgba(0,0,0,0.22)] sm:px-4">
